...
(
  horse_id        INT          NOT NULL COMMENT '競走馬ID（netkeiba準拠）',
  name            INT          NULL     COMMENT '名前',
  birthday        INT          NULL     COMMENT '誕生日',
  home            VARCHAR(128) NULL     COMMENT '産地',
  farm            VARCHAR(128) NULL     COMMENT '生産牧場',
  owner           VARCHAR(128) NULL     COMMENT '馬主',
  training_center INT          NULL     COMMENT 'トレーニングセンター（美穂・栗東）',
  trainer         VARCHAR(128) NULL     COMMENT '調教師',
  father_id       INT          NULL     DEFAULT 0 COMMENT '父',
  mother_id       INT          NULL     DEFAULT 0 COMMENT '母',
  PRIMARY KEY (horse_id)
) COMMENT '競走馬情報';

CREATE TABLE race_horse_result
(
  race_id           INT          NOT NULL COMMENT '管理ID（レース日程+会場ID+レース番号）',
  ...
  weight            INT          NULL     COMMENT '斤量',
  odds              INT          NULL     COMMENT 'オッズ',
  rank              INT          NULL     COMMENT '順位',
  time              INT          NULL     COMMENT 'タイム',
  popularity        INT          NULL     COMMENT '人気',
  last_3f           INT          NULL     COMMENT '上がり3F',
  Jockey            VARCHAR(128) NULL     COMMENT '騎手',
...
(
  ...
  race_condition   INT NULL     COMMENT 'レース条件',
  age              INT NULL     COMMENT '馬齢',
  class            INT NULL     COMMENT 'クラス',
  PRIMARY KEY (race_id)
) COMMENT 'レース基本情報テーブル';

ALTER TABLE race_info
  ADD CONSTRAINT FK_race_date_TO_race_info
    FOREIGN KEY (date)
    REFERENCES race_date (date);

ALTER TABLE race_horse_result
  ADD CONSTRAINT FK_race_info_TO_race_horse_result
    FOREIGN KEY (race_id)
    REFERENCES race_info (race_id);

ALTER TABLE race_horse_result
  ADD CONSTRAINT FK_race_horse_info_TO_race_horse_result
    FOREIGN KEY (horse_id)
    REFERENCES race_horse_info (horse_id);